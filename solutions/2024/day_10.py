...
def part_1(lines):
    return sum([iterative_dfs((x, y), lines, "part_1") for x in range(len(lines)) for y in range(len(lines[0])) if lines[x][y] == '0'])

def part_2(lines):
    return sum([iterative_dfs((x, y), lines, "part_2") for x in range(len(lines)) for y in range(len(lines[0])) if lines[x][y] == '0'])
