...
def part_1(input):
    ...
    def key(item):
        hand, _ = item
        return (sum(hand.count(card) for card in hand), [cards.index(card) for card in hand])
...
def part_2(inpu):
    ...
    def key(item):
        ...
        return (sum(sub.count(card) for card in sub), [cards.index(card) for card in hand])
