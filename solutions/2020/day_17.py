...
def get_next_state(state):
    ...
    next_state = [[['.' for _ in range(new_W)] for _ in range(new_H)] for _ in range(new_D)]
    for d in range(new_D):
        for h in range(new_H):
            for w in range(new_W):
                cnt_active = count_activ_neighboors(state, (d, h, w))
                current_state = state[d - 1][h - 1][w - 1] if (1 <= d < D + 1 and 1 <= h < H + 1 and 1 <= w < W + 1) else '.'
                match current_state:
                    case '#':
                        next_state[d][h][w] = '#' if (cnt_active == 2 or cnt_active == 3) else '.'
                    case '.':
...
def neighbour_coordinates(p):
    return [tuple(a+b for a, b in zip(p, t)) for t in product([-1, 0, 1], repeat=len(p)) if any(t)]

def simulate(dimensions, seed):
    active_coordinates = {(x,y)+(0,)*(dimensions-2) for y, l in enumerate(seed) for x, c in enumerate(l) if c == '#'}
    for _ in range(6):
