...
def neighbour_coordinates(p):
    return [tuple(a + b for a, b in zip(p, t, strict=False)) for t in product([-1, 0, 1], repeat=len(p)) if any(t)]


def simulate(dimensions, seed):
    active_coordinates = {
        (x, y) + (0,) * (dimensions - 2) for y, l in enumerate(seed) for x, c in enumerate(l) if c == "#"
    }
    for _ in range(6):
