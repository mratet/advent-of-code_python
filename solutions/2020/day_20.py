...
def match_tiles(tile_1, tile_2):
    return (
        tile_1[0] == tile_2[-1] or
        tile_1[-1] == tile_2[0] or
        rot_90(tile_1)[0] == rot_90(tile_2)[-1] or
        rot_90(tile_1)[-1] == rot_90(tile_2)[0]
    )
...
def count_sea_monsters(full_image):
    ...
    for i in range(image_width - monster_width + 1):
        for j in range(image_size - monster_size + 1):
            monster_match = [full_image[i + k][j + l] for k in range(monster_width) for l in range(monster_size) if
                             SEA_MONSTERS[k][l] == '#']
...
def get_adjacent_tiles(tiles):
    ...
    for t_id1, t_id2 in product(tiles, repeat=2):
        if t_id1 == t_id2:
            continue
        for (mode_id1, v1), (mode_id2, v2) in product(tiles[t_id1].items(), tiles[t_id2].items()):
...
def part_2(lines):
    init_tiles = parse_tiles(lines)
    tiles = {tile_id : get_all_image_variations(tile) for (tile_id, tile) in init_tiles.items()}
    borderless_tiles = {tile_id: {mode: remove_border(tile) for mode, tile in tiles[tile_id].items()} for tile_id in tiles}
