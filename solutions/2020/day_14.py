...
def solve(input, part):
    ...
    for line in input:
        if line[:3] == "mem":
            ...
            if part == "part_1":
                binary_val = [
                    "1" if val == "1" else "0" for val in format(int(val), "36b")
                ]
                next_val = [
                    mask_val if mask_val != "X" else bin_val
                    for (mask_val, bin_val) in zip(bitmask, binary_val)
                ]
                memory[idx] = int("".join(next_val), 2)
            elif part == "part_2":
                binary_idx = [
                    "1" if val == "1" else "0" for val in format(int(idx), "36b")
                ]
                next_val = [
                    mask_val if mask_val != "0" else bin_val
                    for (mask_val, bin_val) in zip(bitmask, binary_idx)
                ]
                ...
                for i in range(2**n):
                    ...
                    bits = [
                        "1" if val == "1" else "0" for val in format(i, f"{str(n)}b")
                    ]
                    for bit in bits:
