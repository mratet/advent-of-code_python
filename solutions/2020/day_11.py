...
def get_next_state(grid, part):
    ...
    for i in range(len(grid)):
        ...
        for j in range(len(grid[0])):
            ...
            match grid[i][j]:
                case "L":
                    new_line.append("#" if "#" not in view else "L")
                case "#":
                    new_line.append("L" if view.count("#") >= OCCUPIED_TOLERANCE else "#")
                case ".":
                    new_line.append(".")
...
def get_occupied_seats(lines, part):
    grid = lines
    while True:
...
def part_1(lines):
    return get_occupied_seats(lines, "part_1")


def part_2(lines):
    return get_occupied_seats(lines, "part_2")
