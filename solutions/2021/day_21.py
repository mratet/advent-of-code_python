...
GAMEBOARD_SIZE = 10
ROLL_FREQS = Counter(sum(d) for d in product([1, 2, 3], repeat=3))
...
def play_quantum_game(p1_position, p1_score, p2_position, p2_score, turn):
    ...
    if p2_score >= 21:
        return (0, 1)
    total_wins = (0, 0)
    for roll_sum, freq in ROLL_FREQS.items():
        if turn == 0:
            new_p1_position = (p1_position + roll_sum) % GAMEBOARD_SIZE
            new_p1_score = p1_score + new_p1_position + 1
            child_wins = play_quantum_game(new_p1_position, new_p1_score, p2_position, p2_score, 1)
        else:
            new_p2_position = (p2_position + roll_sum) % GAMEBOARD_SIZE
            new_p2_score = p2_score + new_p2_position + 1
            child_wins = play_quantum_game(p1_position, p1_score, new_p2_position, new_p2_score, 0)
        total_wins = total_wins[0] + freq * child_wins[0], total_wins[1] + freq * child_wins[1]
    return total_wins
...
def part_1(lines):
    ...
    p1_position, p2_position = parse_input(lines)
