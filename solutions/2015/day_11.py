...
def part_1(password):
    sequence = lambda s: any(ord(a) + 2 == ord(b) + 1 == ord(c) for a, b, c in zip(s, s[1:], s[2:], strict=False))
    doublets = lambda s: len({a for a, b in itertools.pairwise(password) if a == b}) > 1
    ...
    while True:
        ...
        if password:
            index, char = next(
                ((i, c) for i, c in enumerate(password) if c in "ilo"),
                (-1, password[-1]),
            )
